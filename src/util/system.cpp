...
const char * const BITCOIN_CONF_FILENAME = "torchcoin.conf";
...
#else
    const char* pszModule = "torchcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Torchcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Torchcoin";
#else
    // Unix
    return pathRet / ".torchcoin";
#endif
#endif
}
